...
# 함수는 매개변수를 가질 수 있고, 반환값을 가질 수 있음(반환값이 없을 수 있음)
# 식별자는 스네이크 표기법으로 작성하고, 식별자 뒤에 ()소괄호가 있음
# 일반적으로 매개변수(parameter)와 함수의 호출하는 인자(argument)의 갯수가 일치해야함.
# def 키워드를 사용해서 정의
...
# name_card("가을", "서울시 강남구 신사동", "010-1234-1234", "싱어")
...
def deposit(balance, in_val): # 잔액과 입금을 매개변수로 전달 받음
    print(f"{in_val} 입금되었습니다. 잔액은 {balance + in_val} 입니다.")
    return balance + in_val # 기존 잔액에서 입금 금액을 더해서 반환

def withdraw(balance, out_val): # 잔액과 출금을 매개변수로 전달 받음
    if balance >= out_val:
        print(f"{out_val}이 출금되었습니다. 잔액은 {balance - out_val}")
        return balance - out_val
    else:
        print(f"출금이 실패했습니다. 잔액은 {balance}")
        return balance
...
# 기본값 인자 : 함수 선언시 매개 변수에 대한 기본값을 정의
# 매개변수에 기본값이 정의되어 있는 경우 호출시 인자값을 넣지 않으면 기본값으로 호출
def profile(name, grade = 2, age = 18, school = "태양고"): # 출력 순서는 여기 line # 파이썬은 오버로딩이 없다, 필요 없다, 디폴트 매개변수가 존재
...
# 가변 매개변수 : 함수의 매개변수 앞에 *(별표)를 붙이면 인자값으로 몇개를 전달하든, 튜플로 인식해서 처리 가능 => *(별표)를 찍으면 "전부다"라는 의미
...
# 파이썬의 경우는 함수 스코프 기반의 언어, 외부에서 선언한 변수의 값을 함수내에서 변경하기 위해서는
# global 키워드를 사용해야 함
...
def std_weight(height, gender):
    hm = height / 100   # 입력받은 cm키를 미터 단위로 변경
    if gender == "남성":
        return hm * hm * 22
    else :
        return hm * hm * 21
